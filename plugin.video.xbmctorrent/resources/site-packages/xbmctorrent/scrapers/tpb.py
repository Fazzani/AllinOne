...
IMMUNICITY_TPB_URL = "http://thepiratebay.to/"
...
@ensure_fanart
@tracked
def piratebay_page(root, page=1):
    import re
    ...
    page = int(page)
    html_data = url_get(urljoin(BASE_URL, "%s/27/%d/0/0/0/DDate/%d/" % ("browse",50, 1)), headers=HEADERS)
    #browse/27/porn-movies http://thepiratebay.to/browse/27/50/0/0/0/DDate/1/
    plugin.log.error('_________________' + urljoin(BASE_URL, "%s/27/%d/0/0/0/DDate/%d/" % ("browse",50, 1)))
