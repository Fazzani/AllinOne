...
def library_isExistingItem(item, content_type="movies", isSaison=False):
    import os
    import xbmc

    infos = item.setdefault("info", {})
    content_type = 'tvshows' if isSaison or infos.get("tvshowtitle") else 'movies'
    for name, entry in LIBRARY_PATHS.items():
        if entry["strContent"] == content_type:
             real_path = xbmc.translatePath(entry["strPath"])
             ...
             if content_type == "tvshows":
                 real_path = os.path.join(os.path.join(real_path,properties.get("tvShowName"),properties.get("saisonNumber")), item["label2"])
...
def library_add_new_item(item, isSaison=False):
    import os
    import xbmc
    ...
    for name, entry in LIBRARY_PATHS.items():
        if entry["strContent"] == content_type:
            real_path = xbmc.translatePath(entry["strPath"])
            properties = item.setdefault("properties", {})
            item["label2"] = properties.get("label2").encode('latin-1') if properties.get("label2") else item["label"].encode('latin-1')
            plugin.log.info("________path : %s" % item["label2"])
            if not item["label2"]:
                plugin.notify("Unable to add this file.")
                return
            if  content_type == 'tvshows':
                sousPath = os.path.join(real_path,properties.get("tvShowName"),properties.get("saisonNumber"))
                plugin.log.info('Sous Path : %s' % sousPath)
                real_path = os.path.join(sousPath, item["label2"])
                if not os.path.exists(real_path):
                    os.makedirs(real_path)
