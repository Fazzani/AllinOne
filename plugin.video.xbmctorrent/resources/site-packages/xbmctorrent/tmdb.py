...
def tmdb_config():
    ...
    return url_get_json("%s/configuration" % BASE_URL, params={"api_key": API_KEY}, headers=HEADERS, with_immunicity=False, method='GET')
    ...
    with shelf("com.imdb.%s" % imdb_id) as movie:
        if not movie:
            try:
                import urllib2
                from xbmctorrent.utils import url_get_json
                movie.update(url_get_json("%s/movie/%s" % (BASE_URL, imdb_id), params={"api_key": API_KEY, "append_to_response": "credits"}, headers=HEADERS, with_immunicity=False, method='GET') or {})
            except urllib2.HTTPError:
                pass
        return dict(movie)

def getById(imdb_id,type='movie'):#type: movie, tv
    from xbmctorrent.utils import url_get_json
    
    return url_get_json("%s/%s/%s" % (BASE_URL, type, imdb_id), params={"api_key": API_KEY, "append_to_response": "credits"}, headers=HEADERS, with_immunicity=False, method='GET') or {}
...
def search(query, **kwargs):
    ...
    return url_get_json("%s/search/movie" % BASE_URL, params=kwargs, headers=HEADERS, with_immunicity=False, method='GET')

def searchTv(query, **kwargs):
    from xbmctorrent.utils import url_get_json
    kwargs["query"] = query
    kwargs["api_key"] = API_KEY
    
    return url_get_json("%s/search/tv" % BASE_URL, params=kwargs, headers=HEADERS, with_immunicity=False, method='GET')
