...
TORRENT2HTTP_TIMEOUT = 60
TORRENT2HTTP_POLL = 1000
PLAYING_EVENT_INTERVAL = 60
MIN_COMPLETED_PIECES = 20.0
...
class TorrentPlayer(xbmc.Player):
    def init(self, uri):
        ...
        #        plugin.notify("Downloading to FAT32 is not supported.
        #        Resetting.", delay=15000)
        #        plugin.set_setting("dlpath", "")
        #        self.torrent2http_options["dlpath"] = "."
    ...
    def getMinSize(self, fileSize):
        if fileSize < 380000000:
            return 25
        if fileSize < 780000000:
            return 17
        if fileSize < 1480000000:
            return 14
        return 7
    ...
    def loop(self):
        ...
        with closing(torrent2http.start(**self.torrent2http_options)) as t2h_instance:
            t2h = lambda cmd: url_get_json("http://%s/%s" % (t2h_instance.bind_address, cmd), with_immunicity=True)
            ...
            with closing(SafeDialogProgress(delay_create=0)) as dialog:
                ...
                while not has_resolved:
                    ...
                    plugin.log.info('state : %s' % status["state"])
                    if status["state"] >= 0:
                        dialog.update(int(status["progress"] * 100), *self._get_status_lines(status))
                    
                    if status["state"] >= 3 and not has_resolved: # Downloading?
                        files = t2h("ls")["files"]
                        plugin.log.info('+++++++++++++' + repr(files))
                        biggest_file = sorted(files, key=lambda x: x["size"])[-1]
                        biggest_file["name"] = biggest_file["name"].encode("utf-8")
                        if status["state"] >= 4:
                            itemPath = xbmc.validatePath(os.path.join(xbmc.translatePath(plugin.get_setting("dlpath")), biggest_file["name"]))
                            plugin.log.info('++++++++++++++'+repr(itemPath))
                            item = self.getItem(itemPath)
                            plugin.log.info(repr(item))
                            plugin.set_resolved_url(item)
                            break
                        #percent_complete =
                        #float(biggest_file["complete_pieces"]) /
                        #float(biggest_file["total_pieces"]) * 100.0
                        #if percent_complete >=
                        #self.getMinSize(biggest_file["size"]):
                        #    plugin.log.info("Resolving to http://%s/files/%s"
                        #    % (t2h_instance.bind_address,
                        #    biggest_file["name"]))
                        #    has_resolved = True
                        #    item = self.getItem("http://%s/files/%s" %
                        #    (t2h_instance.bind_address,
