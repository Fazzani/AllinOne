    ...
    def _fn(*a, **kwds):
        ...
        for item in items:
            if item["is_playable"]:
                 ...
                 item["path"] = plugin.url_for("freebox_addByUrl", uri=item["path"])
        return items
    return _fn
...
def tvShowSaisonPathExtract(fn):
    ...
    def _fn(*a, **kwds):
        ...
        for item in items:
            ...
            item.setdefault("label2", "")
            if not item["is_playable"] and item["label2"] is not None:
                 tab = item["label2"].split('=')
                 if len(tab) > 2:
                    item["path"] = plugin.url_for("omg2_saisons", serieName= tab[0].encode("utf-8"), serieId= tab[1].encode("utf-8"), saisonCount=tab[2].encode("utf-8"))
                    item["label2"] = tab[0].encode("utf-8")
        return items
    return _fn
